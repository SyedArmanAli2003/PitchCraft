...
const TOOL_BADGE: Record<ToolKey, { label: string; bg: string; color: string; border: string }> = {
  ...
  mongodb:                  { label: "MONGODB",    bg: "rgba(16,185,129,0.14)", color: "rgb(52,211,153)",    border: "rgba(16,185,129,0.3)"  },
  "free-gateway":           { label: "FREE GATEWAY", bg: "rgba(99,102,241,0.14)", color: "hsl(239,84%,80%)", border: "rgba(99,102,241,0.35)" },
  "nvidia-llama":           { label: "NVIDIA NIM",  bg: "rgba(34,197,94,0.14)", color: "rgb(74,222,128)",   border: "rgba(34,197,94,0.3)"  },
  "nvidia-nemotron":        { label: "NVIDIA NEMOTRON", bg: "rgba(34,197,94,0.16)", color: "rgb(134,239,172)", border: "rgba(34,197,94,0.4)" },
...
function StepData({ stepNumber, data }: { stepNumber: number; data: Record<string, unknown> }) {
  ...
  if (stepNumber === 2) {
    ...
    const raw = data.mongodb_sources ?? data.insforge_sources
    ...
    const sourcesLine = Array.isArray(raw)
      ...
      : raw && typeof raw === "object"
        ? `market_data.${industry ?? "industry"} · ${(raw as Record<string, unknown>).similar_plans_found ?? 0} similar plans matched in MongoDB`
    ...
    return (
      <div>
        ...
        {/* MongoDB MCP visibility panel — the agent queried MongoDB over the real MCP protocol */}
        <div className="mt-3 rounded-xl p-3"
          style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.2)" }}>
          <p className="text-xs font-semibold mb-1.5" style={{ color: "rgb(52,211,153)" }}>
            ◆ MongoDB — market data retrieved
          </p>
  ...
  if (stepNumber === 5) return (
    <div className="space-y-2">
      ...
      {!!(data.mongodb_benchmarks ?? data.insforge_benchmarks) && (
        <p className="text-xs mt-2 flex items-center gap-1.5" style={{ color: "rgba(52,211,153,0.8)" }}>
          ...
          {(() => {
            const bm = (data.mongodb_benchmarks ?? data.insforge_benchmarks) as Record<string, unknown>
